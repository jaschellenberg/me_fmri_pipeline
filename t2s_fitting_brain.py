...
print('processing scan ' + case_dir + ' and sequence number ' + str(nr_me) + ' with ' + str(num_echos) + ' echos, current dynamic: ' + str(dynamic) + '.')
print()
...
for echo in range(int(num_echos)):
    echo_location = nifti_directory + 'recon_struct_brain_e0' + str(echo) + '_dyn' + str(dynamic) + '.nii.gz'
...
t2map_path =  nifti_directory + 't2map_' + str(dynamic) + '_from_recon_brain.nii.gz'

for x,file in enumerate(echo_filenames):
    echo_img = nib.load(file)
    nifti_echo_img = echo_img.get_fdata()
    nifti_echo_img = nifti_echo_img[...,0]
...
print('NIFTI concatenated shape:')
...
#dyn=nifti_concat.shape[3]

te = get_TE.read(dicom_directory)
te = te[0:3]

print('TEs & dimensions:')
print(te)
